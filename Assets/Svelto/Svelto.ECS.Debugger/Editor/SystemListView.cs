...
namespace Svelto.ECS.Debugger.Editor
{
    ...
    internal delegate void SystemSelectionCallback(DebugGroup manager, DebugRoot root);
...
//        }
        ...
        private HideNode CreateNodeForManager(int id, DebugGroup system)
        {
            ...
            active = false;
            var name = getWorldSelection() == null ? $"{system.GetType().Name} ({system.Parent.Name})" : system.GetType().Name;
        ...
        private HideNode BuildNodesForComponentSystem(DebugGroup manager, ref int currentId)
        {
            switch (manager)
            {
        ...
        protected override void RowGUI (RowGUIArgs args)
        {
            ...
            if (managersById.ContainsKey(item.id))
            {
                ...
                var componentSystemBase = manager as DebugGroup;
        ...
        public bool NeedsReload
        {
            get
            {
                ...
                {
                    if (manager is DebugGroup system)
                    {
                        if (system.DebugRoot == null || !system.World.Systems.Contains(manager))
                            return true;
                    }
                }
