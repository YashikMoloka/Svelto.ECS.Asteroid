...
namespace Svelto.Tasks.Unity.Internal
{
    class RunnerBehaviourUpdate : MonoBehaviour
    {
        ...
        {
            ExecuteRoutines(_renderProcesses);
        }
        ...
        public void StartRenderCoroutine(IProcessSveltoTasks enumerator)
        {
            _renderProcesses.Add(enumerator);
        }

        readonly WaitForEndOfFrame _waitForEndOfFrame = new WaitForEndOfFrame();

        readonly FasterListThreadSafe<IProcessSveltoTasks> _renderProcesses    = new FasterListThreadSafe<IProcessSveltoTasks>();
